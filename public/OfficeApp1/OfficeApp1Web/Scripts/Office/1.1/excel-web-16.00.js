...
OSF.WebApp.AddXdmInfoAsHash = function OSF_WebApp$AddXdmInfoAsHash(url) {
    return OSF.OUtil.addXdmInfoAsHash(url, OSF._OfficeAppFactory.getWebAppState().conversationID + "|" + OSF._OfficeAppFactory.getId() + "|" + OSF._OfficeAppFactory.getWebAppState().webAppUrl);
};

OSF.WebApp._UpdateLinksForXdmInfo = function OSF_WebApp$_UpdateLinksForXdmInfo() {
    var links = document.getElementsByTagName("a");
    for (var i = 0; i < links.length; i++) {
        if (OSF.WebApp._isGoodUrl(links[i].href)) {
            links[i].href = OSF.WebApp.AddXdmInfoAsHash(links[i].href);
        }
    }
    ...
    for (var k = 0; k < forms.length; k++) {
        ...
        if (OSF.WebApp._isGoodUrl(form.action)) {
            form.action = OSF.WebApp.AddXdmInfoAsHash(form.action);
        }
    }
};
...
OSF.InitializationHelper.prototype.setAgaveHostCommunication = function OSF_InitializationHelper$setAgaveHostCommunication() {
    OSF.WebApp._UpdateLinksForXdmInfo();
    var me = this;
